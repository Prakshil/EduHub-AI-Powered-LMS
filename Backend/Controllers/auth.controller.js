...
export const requestOtp = async (req, res) => {
  try {
    ...
    if (process.env.NODE_ENV !== 'production') console.log(`Generated OTP for ${email}: ${otp}`);

    // Save OTP to database first
    await createOrUpdateOtp(email, otp);
    if (process.env.NODE_ENV !== 'production') console.log(`OTP saved to database for ${email}`);
...
export const validateOtp = async (req, res) => {
  try {
    ...
    if (process.env.NODE_ENV !== 'production') console.log(`Validating OTP for ${email}: ${otp}`);
    ...
    if (process.env.NODE_ENV !== 'production') console.log(`OTP verification result for ${email}: ${ok}`);
    ...
    if (user) {
      ...
      if (process.env.NODE_ENV !== 'production') console.log(`User ${email} verified and logged in`);
    ...
    if (process.env.NODE_ENV !== 'production') console.log(`OTP verified for ${email} but no user found`);
